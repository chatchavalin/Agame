...
(function () {
  ...
  function renderTile(tile, faceDown, onBoard) {
    ...
    // Face-up tile
    // On the board, assigned choice tiles (×/÷ → ×, +/- → +, etc.) should be
    // visually indistinguishable from regular operator tiles. Use the CSS class
    // that matches the assigned face instead of 'tile-choice'.
    if (onBoard && tile.type === 'choice' && tile.assigned) {
      const ops = new Set(['+', '-', '×', '÷']);
      if (ops.has(tile.assigned)) {
        el.classList.add('tile-op');
      } else if (tile.assigned === '=') {
        el.classList.add('tile-equals');
      } else {
  ...
  function buildGameLayout(root) {
    ...
    buttonBar.innerHTML = `
      ...
      <button class="btn btn-icon" id="btn-lobby" title="Back to Lobby" onclick="(window.AMath&&window.AMath.exitToLobby)?window.AMath.exitToLobby():(window.location.href='lobby.html')">🏠</button>
